...
> Versión del documento: `v1.10.0` · Última actualización: 2026-08-05
...
analytics_events  id, kind ('page_view'|'product_view'|'whatsapp_click'
                  |'agent_query'), path, product_id (nullable),
                  session_id, meta jsonb, created_at
```
...
| Rediseño "Pizarra" | 🟢 Código terminado | 2026-08-06 | Home + pasada de coherencia sobre listado, detalle, carrito y panel (decisiones 70 y 71). Card única, píldora única, contenedor único, todo número en mono. Verificado: build limpio, sin overflow a 375/768/1024/1440, el botón de comprar recibe el toque (hit-test), táctiles ≥44 px, sin errores de consola. **Falta:** teléfono real con datos móviles, Lighthouse móvil, y el panel logueado sin verificar visualmente (requiere sesión). Fase 4 (copy) descartada por decisión del dueño |
| ~~Rediseño (detalle histórico)~~ | 🟡 En curso | 2026-08-06 | Rama `feat/rediseno-pizarra` (**sin push**), referencia en `design/`. **Hecho:** tokens ámbar + hueso con alias de los nombres viejos, IBM Plex Mono, `brand.config.ts`, `manifest.json` (fase 1). Home completa: pizarra split-flap con datos reales, marquee de marcas del catálogo, bento, destacados con tilt, pasos y promesas; `GET /api/rate`; motion propio sin Framer Motion (fases 2 y 3). Decisiones 61 a 68. Verificado en navegador: sin scroll horizontal a 375/768/1024/1440, y con la cotización quieta la pizarra no anima. **Falta:** prueba en teléfono real, Lighthouse móvil, y la pasada de coherencia sobre listado, detalle, carrito y panel — hoy siguen con el sistema viejo repintado por los alias. **Fase 4 (copy) descartada por decisión del dueño:** queda pendiente que el footer y la franja de confianza dicen "Envíos a todo el país" mientras la home promete "Buenos Aires y GBA" |
| F6 Pulido | ⚪ Pendiente | — | |
...
| 46 | **El carrito guarda USD, no pesos** | Es la misma regla del catálogo (§4) llevada a localStorage. Un carrito abandonado ayer mostraría el precio de ayer mientras la ficha del mismo producto muestra el de hoy, y el cliente ve dos números distintos del mismo ítem. El peso se calcula al pintar y recién se congela al confirmar el pedido (`usd_rate_snapshot`) | 2026-08-05 |
| 47 | `POST /api/orders` **ignora los precios que manda el navegador** | El body lleva solo IDs y cantidades; los precios se releen de la base y se convierten en el servidor. localStorage lo edita cualquiera con las devtools abiertas, y un pedido guardado con un precio inventado es un problema real cuando el dueño lo lee al otro día. De paso el handler valida stock y que la capacidad y el color sean de ese producto | 2026-08-05 |
| 48 | El código de pedido lo genera la base, con `create_order` (`SECURITY DEFINER`) y una secuencia | El comprador es anónimo: RLS lo deja insertar en `orders` pero no leer, así que un `insert().select()` nunca devolvería el código. Calcularlo en la app obligaría a leer el último pedido —que anon no puede— y abriría una carrera entre dos compradores simultáneos. El prefijo llega **por parámetro** desde `brand.config.ts`: escribir `IPX` en la base rompería la decisión 07. Queda anotado que el RPC es público, igual que ya lo era el INSERT de F2: el endurecimiento (rate limit o alerta de pedidos basura) entra en F7 junto con n8n | 2026-08-05 |
| 49 | En `/carrito` el botón de enviar va al final del formulario, no en barra fija | Dos razones de celular: el teclado virtual tapa las barras fijas justo cuando el cliente termina de escribir el teléfono, y el borde inferior ya está ocupado por el widget de Retell y el botón de WhatsApp (decisión 38). Al final del formulario queda igual en la zona del pulgar, justo debajo del último campo. Corrige el wireframe de SPEC.md §5 | 2026-08-05 |
| 50 | El descuento por transferencia se aplica al precio **unitario** y después se multiplica | Es por producto, no global: un accesorio y un iPhone pueden tener porcentajes distintos. Aplicándolo al total de la línea, el total de transferencia no coincidiría con el precio unitario que muestra esa misma línea en pantalla. Por eso `cartTotals()` cambió de firma y `orderCode()` se eliminó de `pricing.ts` | 2026-08-05 |
| 51 | La barra de acción del detalle sube a z-index 1000001 y su contenedor deja de animarse; el botón flotante de WhatsApp no se dibuja en la ficha | Encontrado probando F4: "Agregar al carrito" se veía pero **no respondía al toque**. Dos causas encadenadas. (a) El `_fabWrapBase` de Retell tapa la franja inferior entera con `pointer-events: auto` y z-index 999999 — la misma trampa de la decisión 38, que esta vez se comió el botón principal de la tienda. (b) El contenedor tenía `.reveal`, y un ancestro animado rompe un `position: fixed` de dos formas: el `transform` lo vuelve su bloque contenedor (la barra quedaba al final de la ficha, fuera de la zona del pulgar) y la animación de `opacity` con `fill-mode: both` le crea un contexto de apilamiento permanente que encierra su z-index. Verificado con hit-testing a 375 px contra el build de producción. El flotante se saca de la ficha porque la barra ya tiene su propio botón de WhatsApp: quedaban duplicados y el de abajo tapado. **Efecto lateral asumido:** en la ficha de producto la burbuja de Retell queda detrás de la barra. Se prioriza que el botón de comprar funcione; el asistente sigue disponible en todas las demás pantallas | 2026-08-05 |
| 52 | El panel es **una sola pantalla**: métricas arriba, precios abajo | El dueño entra a mirar cómo viene el día y de paso corrige un precio. Repartirlo en dos pantallas le agrega navegación a la tarea más frecuente del sistema. El resto del CRUD (crear producto, subir fotos, editar variantes) queda en sus propias pantallas porque es trabajo de una vez por mes, no de todos los días | 2026-08-05 |
| 53 | El precio se edita **en USD**, con el peso calculado al lado en vivo | Es la decisión 03 llevada al panel. Un campo "precio en pesos" dejaría los 42 precios viejos al día siguiente — el dolor que este proyecto existe para resolver. Mostrar el peso mientras se tipea da lo bueno de las dos cosas: el dueño ve el número que va a ver el cliente antes de guardar, y en la base sigue habiendo solo dólares | 2026-08-05 |
| 54 | La tienda **no** usa Supabase Realtime; se revalida on-demand | Para que la vitrina reaccione sola habría que volver cliente cada ficha y suscribirla a un websocket: ~40 KB de bundle más una conexión abierta con datos móviles, contra el presupuesto de 500 KB y 3 s de §1. `revalidatePath("/", "layout")` desde la Server Action da el mismo resultado visible con costo cero para el comprador, y encima instantáneo (el ISR de 60 s se saltea). Realtime queda disponible para el propio panel, donde el presupuesto no aplica | 2026-08-05 |
| 55 | Los eventos se escriben por el RPC `track_event`, no por INSERT anónimo | Con una política de INSERT para anon, cualquiera podía mandar rutas externas, `meta` de cualquier tamaño y `product_id` inventados. El RPC (`SECURITY DEFINER`, como `create_order`) valida tipo de evento, exige que la ruta sea interna, verifica que el producto exista y le pone techo al `meta`. La tabla no tiene política de INSERT para nadie: ese es el único camino | 2026-08-05 |
| 56 | `revoke ... from public` **no** le saca el permiso a `anon` | Supabase le da EXECUTE nominal a `anon` sobre las funciones nuevas del schema `public` por default privileges, y un revoke al pseudo-rol PUBLIC no toca ese grant. `dashboard_metrics` quedó llamable sin sesión —sin fuga, porque es `SECURITY INVOKER` y RLS le tapaba las filas, pero igual era un endpoint abierto haciendo trabajo. Hay que revocar de `anon` por nombre | 2026-08-05 |
| 57 | Las Server Actions verifican la sesión aunque ya la verifiquen el proxy y RLS | No es redundante: un `update` de PostgREST que no matchea ninguna fila **no devuelve error**. Sin el chequeo, una acción sin sesión respondía "Precio actualizado" sin haber escrito nada. Un cambio que no pasó y el dueño creyendo que sí es peor que un error visible | 2026-08-05 |
...
| 64 | Se agrega `GET /api/rate` en vez de que el navegador le pegue a dolarapi | `exchange.ts` es `server-only` y ahí viven la validación de cordura y la cadena de fallback de la decisión 34. Una llamada directa desde el cliente se saltea las dos y mete una API externa en el camino crítico. El endpoint devuelve **los pesos ya calculados**: si el navegador redondeara por su cuenta, el precio cambiaría solo al hidratar. Cacheado 60 s como el ISR, así cien pestañas abiertas no son cien consultas por minuto a São Paulo | 2026-08-06 |
| 65 | La pizarra gira **solo los caracteres que cambiaron** | Es lo que separa un dato de una decoración. Por eso los casilleros se pintan una vez desde React y después los maneja el efecto por DOM: si el texto viviera en el JSX, cada cambio de prop volvería a renderizar los `<span>` en el medio del giro. Verificado: con la cotización quieta, cero celdas animadas | 2026-08-06 |
...
| 70 | **Una sola card, una sola píldora, un solo contenedor**: pasada de coherencia sobre listado, detalle, carrito y panel | El rediseño solo tocó la home, y el resto quedó con el sistema viejo repintado por los alias de token: mismos colores, otra tipografía en los números, otros radios, otros botones. Se arrastraron tres bugs reales. (a) **12 botones con texto blanco sobre ámbar**, 2:1 de contraste — sobre el azul anterior pasaban AA y el cambio de acento los rompió sin que se notara, porque la home no usa ninguno de esos botones; van a `--on-amber`, 13:1. (b) A 1440 el header medía 1320 px y el catálogo 1024: el encabezado sobresalía del contenido. Ahora `page-wrap` usa el mismo `--maxw` y `--pad` que la chrome, y lo que se lee o se completa —ficha y carrito— usa `content-wrap`, angosto a propósito. (c) `--warn` (#F59E0B) era casi el acento, así que un error y un precio destacado se leían igual: pasa a terracota #D97742, 6,3:1. Además `ProductCard` y `ShowcaseCard` eran dos componentes con markup distinto —de ahí que la home y el catálogo no se parecieran—; ahora la card es una sola y el `<Tilt>` es lo único que las separa | 2026-08-06 |
| 71 | El pie y la franja de confianza de la ficha dejan de tener los datos escritos a mano | El footer publicaba un teléfono que no era el del WhatsApp de la tienda y dos correos fijos, y la ficha decía "Envíos a todo el país" aunque se editara desde el panel. Las dos cosas rompían la decisión 07 en silencio: cambiar de marca dejaba el sitio mintiendo. Ahora salen de `brand.config.ts` y de `store_config` | 2026-08-06 |
---
